...
class APFObstacleAvoidance(Node):
    ...
    def filter_points_by_radius(self, points):
        return [
            ...
            if 1.2 < point[1] < 5.0 or np.linalg.norm([point[0],point[2]]) < self.influence_radius
        ]
    ...
    def pointcloud_callback(self, msg):
        ...
        for point in filtered_points:
            ...
            # print(distance)
            if n_pointz> 0.2:
                if distance < self.influence_radius and distance > 0.7:
                    self.closest_obstacles.append((n_pointx, n_pointy))
                    # print("obs_values")
                if distance < self.detection_distance and distance > 0.7:
                    # print("detected.!!!")
        ...
        if self.total_force <1100:
            if not hasattr(self, 'last_record'):
                self.last_record = start_time 
            if time.time() - self.last_record > 2:
                self.path_points.append((self.robot_position.x, self.robot_position.y))
                print(f"Stored Points: x:{self.robot_position.x}, y:{self.robot_position.y}")
                self.last_record = time.time()  
                if len(self.path_points) > 10:
                    self.path_points.pop(0)

        print(f"Path Points (Last 10): {self.path_points}")           
    ...
    def control_loop(self):
        ...
        if self.obstacle_detected:
            ...
            self.total_force = sqrt(force_x**2 + force_y**2)
            # print(f"total_forces:{total_force}")
            curr_ori = self.robot_orientation
            # print(f"cur_or:{curr_ori}")
            if self.total_force > 0 and self.total_force < 1100:
                # print("hi_prior_last")
            ...
            elif self.total_force > 1100:
                print("Total force exceeded threshold. Moving to target points.")
                if not hasattr(self, 'target_points'):
                    self.target_points = self.path_points
                ...
                if self.current_target_index < len(self.target_points):
                    ...
                    angular_error = self.normalize_angle(target_heading - self.robot_orientation)
                    if distance_to_target < 0.2:
                        print(f"Target {self.current_target_index + 1} reached!")
